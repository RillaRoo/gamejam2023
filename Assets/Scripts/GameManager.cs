	...
	void GeneratePuzzle()
	{
		...
		for (int h = 0; h < puzzle.height; h++) {
			...
			for (int w = 0; w < puzzle.width; w++) {
				...
				GameObject go =  (GameObject) Instantiate (piecePrefabs [valueSum], new Vector3 (wIterator, hIterator, 0), Quaternion.identity);
				...
				puzzle.pieces[w, h].w = w;
				puzzle.pieces[w, h].h = h;
				wIterator += 0.6f;
